...
/* This file is for adding the mp3 database entries to the mp3 playlist and for popup setup */
...
{
	global $smcFunc, $txt, $context, $user_info, $sourcedir;
	...
	if ($playlist_id == 989 && $playsong == false)
		die();
	...
	/* Adjust variables for playlist bbc - Only enabled upload playlists will be available or if moderator/admin/permission for all/single playlist ! */
	...
	$result1 = $smcFunc['db_query']('', "SELECT myplaylist, autofile, equip, settings.forumlink, settings.override, settings.reference, settings.filename
		...
		if ($setting['forumlink'] == 1)
		{
			...
			{							
				...
				foreach ($check_user as $user)
				{
					$z = 1;
					$stars[0]  = 0;				
					$result2 = $smcFunc['db_query']('', "SELECT id_group, stars FROM `{db_prefix}membergroups` WHERE `{db_prefix}membergroups`.`id_group` = '$user'");									
			...
			$result2 = $smcFunc['db_query']('', "SELECT id_group, permission FROM `{db_prefix}permissions` 
								WHERE (`{db_prefix}permissions`.`id_group` = '$check_group') AND (`{db_prefix}permissions`.`permission` RLIKE '$columnx')");					
			while ($val = $smcFunc['db_fetch_assoc']($result2))
				$playlistx = ((int)str_replace($columnz, "" , $val['permission']));																		
	...
	if ($playlist_id != 989 && $playsong == false)
	{ 
		/* mp3 entries for regular list */		
		$result1 = $smcFunc['db_query']('', "SELECT tag, playlist, enable, description, url, settings.type FROM {db_prefix}premiumbeat_mp3s 
							LEFT JOIN {db_prefix}premiumbeat_settings AS settings ON (settings.myplaylist = playlist)
							LEFT JOIN {db_prefix}premiumbeat_forum_settings AS forum ON (forum.autoplay = playlist)
							WHERE ((playlist = {$playlist_id}) OR (playlist = 99)) AND (enable = 1) ORDER BY tag ASC");
		while ($val = $smcFunc['db_fetch_assoc']($result1))
		{	
			...
			if (empty($val['filename']))
				$val['filename'] = 'my_music/music';
									
			if (empty($val['autoplay']))
				$val['autoplay'] = 0;
				
			if ((int)$val['playlist'] == (int)$val['autoplay']) 
				continue; 
...
function playlist($playlist, $type, $songcount)
{
	...
	{
		$opt = $songcounter;
		$play .= iconv($context['character_set'], 'UTF-8', urldecode($playlist[$opt])); 		
		$songcounter++;
	}
...
{
	global $scripturl, $smcFunc, $txt, $context, $boarddir, $sourcedir, $user_info, $modSettings;
	...
	$context['premiumbeat_axisX'] = !empty($modSettings['premiumbeat_axisX']) ? $modSettings['premiumbeat_axisX'] : '0';
	$context['premiumbeat_axisY'] = !empty($modSettings['premiumbeat_axisY']) ? $modSettings['premiumbeat_axisY'] : '0';	
	
	/* Adjustable parameters  */	
	$width = !empty($_SESSION['premiumbeat_width']) ? (int)$_SESSION['premiumbeat_width'] : 0;
	$height = !empty($_SESSION['premiumbeat_height']) ? (int)$_SESSION['premiumbeat_height'] : 0;
	$autoplay = !empty($_SESSION['premiumbeat_autoplay']) ? $_SESSION['premiumbeat_autoplay'] : 'yes';
	$override_settings = !empty($_SESSION['premiumbeat_new']) ? $_SESSION['premiumbeat_new'] : 'nil';
	
	$_SESSION['premiumbeat_bbc'] = false;
	if (empty($_SESSION['customMusic_checker'])) {$_SESSION['customMusic_checker'] = false;}
	...
	if ($playlist != 989)
	{
		...
		$result2 = $smcFunc['db_query']('', 'SELECT filename, forumlink, override, autoplay FROM {db_prefix}premiumbeat_forum_settings WHERE reference = 1 LIMIT 1');
		while ($val = $smcFunc['db_fetch_assoc']($result2))
		{						  
			foreach ($columns as $column)
			{
	...
	if ($playlist == 999 || $playlist == 0)
	{
		...
		if ($setting['forumlink'] == 1)
		{
			...
			while ($val = $smcFunc['db_fetch_assoc']($result2))
			{						  
				$playlist = str_replace($columnz, "" , $val['permission']);
	...
	$result2 = $smcFunc['db_query']('', 'SELECT skin, skin_type, override FROM {db_prefix}premiumbeat_forum_settings WHERE reference = 1 LIMIT 1');
	while ($val = $smcFunc['db_fetch_assoc']($result2))
	{						  
		$skin = $val['skin'] ? $val['skin'] : '000000';						
		$skinType = $val['skin_type']? (int)$val['skin_type'] : 5;
		if ((int)$_REQUEST['playlist'] == 989 && $override_settings === 'nil')
			$playlist_id = !empty($val['override']) ? (int)$val['override'] : 999;
	}
	$smcFunc['db_free_result']($result2);	
	...
	{		
		...
		if ($a == true)
		{			
			$result1 = $smcFunc['db_query']('', "SELECT myplaylist, height, width, autoplay, type, skin, skin_type, title FROM {db_prefix}premiumbeat_settings  
								WHERE (myplaylist = {$playlist_id}) LIMIT 1");
			while ($val = $smcFunc['db_fetch_assoc']($result1))
			{   
				if ((empty($val['myplaylist']))) 
					continue;   
...
{
	return urlencode(htmlspecialchars($var));
	$pattern = '/&(#)?[a-zA-Z0-9]{0,};/';       
	if (!$var)
		return false;
