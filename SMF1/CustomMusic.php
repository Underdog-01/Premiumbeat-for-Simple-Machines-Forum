...
function custom_mp3()
{
	global $scripturl, $txt, $context, $sourcedir, $boarddir, $boardurl;	
	...
	$_REQUEST['sa'] = isset($_REQUEST['sa']) && isset($subActions[$_REQUEST['sa']]) ? $_REQUEST['sa'] : "Premiumbeat"; 
	
	// Create the tabs for the template.
	$context['admin_tabs'] = array(
		'title' => $txt['customMusic_tabtitle'],
		'help' => '',
		'description' => $txt['customMusic_tabtitle6'],
		'tabs' => array(),
	);

	if (allowedTo('premiumbeat_config'))
	{
		$context['admin_tabs']['tabs']['BrowsePremiumbeat'] = array(
			'title' => $txt['premiumbeat_browse'],
			'description' => '',
			'href' => $scripturl . '?action=custom_mp3;sa=BrowsePremiumbeat',
			'is_selected' => in_array($_REQUEST['sa'], array('BrowsePremiumbeat')),
		);
		$context['admin_tabs']['tabs']['EditPremiumbeat'] = array(
			'title' => $txt['premiumbeat_edit_add'],
			'description' => '',
			'href' => $scripturl . '?action=custom_mp3;sa=EditPremiumbeat',
			'is_selected' => in_array($_REQUEST['sa'], array('EditPremiumbeat')),		
		);
	...
	{
		$context['admin_tabs']['tabs']['SettingsPremiumbeat'] = array(
			'title' => $txt['premiumbeat_settings'],
			'description' => '',
			'href' => $scripturl . '?action=custom_mp3;sa=SettingsPremiumbeat',
			'is_selected' => in_array($_REQUEST['sa'], array('SettingsPremiumbeat')),		
		);
		$context['admin_tabs']['tabs']['PlaylistPremiumbeat'] = array(
			'title' => $txt['premiumbeat_playlist'],
			'description' => '',
			'href' => $scripturl . '?action=custom_mp3;sa=PlaylistPremiumbeat',
			'is_selected' => in_array($_REQUEST['sa'], array('PlaylistPremiumbeat')),		
		);	
	...
	{
		$context['admin_tabs']['tabs']['UploadPremiumbeat'] = array(
			'title' => $txt['premiumbeat_upload'],
			'description' => '',
			'href' => $scripturl . '?action=custom_mp3;sa=UploadPremiumbeat',
			'is_selected' => in_array($_REQUEST['sa'], array('UploadPremiumbeat')),		
		);
		$_REQUEST['sa'] = isset($_REQUEST['sa']) && isset($subActions[$_REQUEST['sa']]) ? $_REQUEST['sa'] : 'UploadPremiumbeat'; 
	}
	
	$context['admin_tabs']['tabs']['LicensePremiumbeat'] = array(
		'title' => $txt['premiumbeat_license'],
		'description' => '',
		'href' => $boardurl . '/my_music/Premiumbeat_for_SMF-license.pdf',
		'is_selected' => in_array($_REQUEST['sa'], array('LicensePremiumbeat')),
		'is_last' => true,
	);
	...
	$context['post_url'] = $scripturl . '?action=custom_mp3;sa=BrowsePremiumbeat;save';		
	...
	foreach ($list_types as $list_type)
		{						
			if (empty($_REQUEST[$list_type])) {$_REQUEST[$list_type] = false;}
	...
	{			
		if (empty($_REQUEST[$edit_type]))
			$_REQUEST[$edit_type] = false;				
		$context['xtag'][$edit_type] = $_REQUEST[$edit_type];			
	} 
			
	$subActions[$_REQUEST['sa']][0]();	
	adminIndex('premiumbeat');	
...
function BrowseCustomMusic()
{
	...
	$user = (int)$context['user']['id'];
	
	/* Check if the user has a playlist preference */
	if (((int)$context['user']['id']) > 0 && check_user($user))
	{	
		$result1 = db_query("SELECT user_id, last_playlist FROM {$db_prefix}premiumbeat_users WHERE (user_id = '{$user}' AND last_playlist) LIMIT 1", __FILE__, __LINE__);
	...
	{	
		if (!check_user($user))
			$request = db_query("INSERT INTO {$db_prefix}premiumbeat_users (`user_id`, `last_playlist`, `pref_playlist`, `downloads`, `autoplay`, `visibility`) 
												VALUES ('{$user}', '{$context['queryPlaylist']}', '0', '0', '0', '0')", __FILE__, __LINE__);
		else															
			$request =db_query("UPDATE {$db_prefix}premiumbeat_users SET last_playlist = '{$context['queryPlaylist']}'
												 WHERE (user_id = '{$user}')", __FILE__, __LINE__);
	}	
	elseif ($playlist == 0)
	{	
		if (!check_user($user))
		{
			$request = db_query("DELETE FROM {$db_prefix}premiumbeat_users WHERE user_id LIKE {$user}", __FILE__, __LINE__);
			$request2 = db_query("INSERT INTO {$db_prefix}premiumbeat_users (`user_id`, `last_playlist`, `pref_playlist`, `downloads`, `autoplay`, `visibility`) VALUES ('{$user}', '0', '0', '0', '0', '0')", __FILE__, __LINE__);
		}	
												
		else															
			$request = db_query("UPDATE {$db_prefix}premiumbeat_users SET last_playlist = 0 WHERE (user_id = '{$user}')", __FILE__, __LINE__);
	}
	...
	if ($mytag > 0)
		{
			...
			if ($a == false)
				{
					$request = db_query("DELETE FROM {$db_prefix}premiumbeat_mp3s WHERE tag LIKE {$mytag}", __FILE__, __LINE__);
					$request = db_query("INSERT INTO {$db_prefix}premiumbeat_mp3s (`tag`, `playlist`, `enable`, `description`, `url`, `id_user`) 
															VALUES ('{$mytag}' , '1', '1', false, false, '{$user}')", __FILE__, __LINE__);
				}
		}
	...
	foreach ($edit_types as $edit_type)
		{	
			if ($edit_type == 'playlist' && (int)$context['xtag'][$edit_type] > 0)
			{
				...
				if (!check_user($user))
					$request = db_query("INSERT INTO {$db_prefix}premiumbeat_users (`user_id`, `last_playlist`, `pref_playlist`, `downloads`, `autoplay`) 
											VALUES ('{$user}', '{$plist}', '0', '0', '0')", __FILE__, __LINE__);
				else															
					$request = db_query("UPDATE {$db_prefix}premiumbeat_users SET last_playlist = '{$plist}'
											 WHERE (user_id = '{$user}')", __FILE__, __LINE__);	
			}
				
			if (empty($context['xtag'][$edit_type])) {$context['xtag'][$edit_type] = false; continue;}	
			if ($context['xtag']['tag'] > 0 && $context['xtag']['enable'] == 2 && $edit_type == 'enable')	
				{	
					$columnName = 'enable';	
	...
	$result1 = db_query("SELECT tag, playlist, enable, description, url, id_user, settings.title FROM {$db_prefix}premiumbeat_mp3s
						 LEFT JOIN {$db_prefix}premiumbeat_settings AS settings ON (settings.myplaylist = playlist)
						 WHERE {$query} ORDER BY playlist ASC", __FILE__, __LINE__);
	while ($val = mysql_fetch_assoc($result1))
		{	
			if ((empty($val['url'])) || (empty($val['tag']))) {continue;}	 
			if ($val['tag'] == 0) {continue;}	
			if (empty($val['description'])) {$val['description'] = false;} 
	...
	foreach ($check_playlist as $play)
		{
			...
			if ($a == false)
				{
					$request = db_query("DELETE FROM {$db_prefix}premiumbeat_settings WHERE myplaylist LIKE {$play}", __FILE__, __LINE__);
					$request = db_query("INSERT INTO {$db_prefix}premiumbeat_settings (`myplaylist`, `height`, `width`, `autoplay`, `type`, `skin`, `skin_type`, `autofile`, `title`, `equip`) VALUES ({$play} , '215', '200', '1', '0', '000000', '1', false, false, '0')", __FILE__, __LINE__);
				}
	...
	if (isset($_REQUEST['save']))
		{
							...
							foreach ($delete_tags as $check_tag)
								{
									...
									if ((int)$check_tag == $i)
										{																			
											$tableName = 'premiumbeat_mp3s';											
											$request = db_query("DELETE FROM `{$db_prefix}$tableName` WHERE `{$db_prefix}$tableName`.`tag` = '$i'", __FILE__, __LINE__);								
						...
						{							
							...
							foreach ($enable_tags as $check_mp3x)
								{									
									if ((int)$check_mp3x == $i)								
										{																				
											$request = db_query("UPDATE {$db_prefix}premiumbeat_mp3s SET enable = IF(enable=1, 0, 1)
																				 WHERE tag = {$i}", __FILE__, __LINE__);								 
										}	
			...
			elseif (isset($_POST['reset']) && is_array($_POST['reset']))
				{
					$request = db_query("UPDATE {$db_prefix}premiumbeat_users SET last_playlist = 0 WHERE (user_id = '{$user}') LIMIT 1", __FILE__, __LINE__);					
					redirectexit('action=custom_mp3;sa=BrowsePremiumbeat;queryPlaylist=0;');
				}				
			else 
				{$delete_tags = array(); $enable_tags = array(); $track = 0;}		
		 
			redirectexit('action=custom_mp3;sa=BrowsePremiumbeat;');
		}
...
function EditCustomMusic()
{
	...
	global $txt, $scripturl, $context, $db_prefix;
	loadLanguage('Modifications');	
	$context['robot_no_index'] = true;	
	/* if (!allowedTo('premiumbeat_config')) 
	...
	/* Read db entries for the mp3's  */
	$result1 = db_query("SELECT tag, playlist, enable, description, url, id_user FROM {$db_prefix}premiumbeat_mp3s WHERE (tag > 0) ORDER BY tag ASC", __FILE__, __LINE__);
	while ($val = mysql_fetch_assoc($result1))
		{	
			if ((empty($val['url'])) || (empty($val['tag']))) {continue;}	 
			if ($val['tag'] == 0) {continue;}	
	...
	if ($context['track_id'] == 0) 
		{
			...
			$context['playlist'][$count] = 1;
			$context['enable'][$count] = 1;
			$context['description'][$count] = false;
			$context['url'][$count] = false;
			$context['id_user'][$count] = 0;	
		}		
	$context['post_url'] = $scripturl . '?action=custom_mp3;sa=BrowsePremiumbeat;edit=' . $context['number_tag'] . ';save;';
	$context['edit_title'] = $txt['customMusic_tabtitle4'];	
...
function SettingsCustomMusic()
	{
	global $txt, $scripturl, $context, $db_prefix, $sourcedir;	
	loadLanguage('Modifications');	
	$context['sets'] = array();
	$context['robot_no_index'] = true;		
	if (!empty($context['queryPlaylist']))
	{			
		if ((int)$context['queryPlaylist'] > 0) 
			redirectexit('action=custom_mp3;sa=BrowsePremiumbeat;queryPlaylist='.(int)$context['queryPlaylist']);			
	}			
	$a = false;
	$b = false;
	$count = 0;
	...
	$permission_sets = array('ID_GROUP', 'permission', 'addDeny');
	...
	foreach ($context['deletePL'] as $delete)
	{
		...
		$delete = (int)$delete;	
		$request = db_query("DELETE FROM {$db_prefix}premiumbeat_settings WHERE myplaylist LIKE '{$delete}'", __FILE__, __LINE__);
		$request = db_query("DELETE FROM {$db_prefix}premiumbeat_mp3s WHERE playlist LIKE '{$delete}'", __FILE__, __LINE__);
		$request = db_query("DELETE FROM {$db_prefix}premiumbeat_data WHERE my_playlist LIKE '{$delete}'", __FILE__, __LINE__);
			...
			if ($context['lists'][$list_type][0] == true && $check_list > 0) 
				{					
					...
					if ($list_type == 'autoplay')
						{
							...
							$request = db_query("UPDATE {$db_prefix}premiumbeat_settings SET autoplay = {$value}
																				 WHERE myplaylist = {$check_list}", __FILE__, __LINE__);
							continue;
						}
					elseif ($list_type == 'equip')
						{
							if ((int)$value > 1) {$value = 0;}
							$request = db_query("UPDATE {$db_prefix}premiumbeat_settings SET equip = {$value}
																				 WHERE myplaylist = {$check_list}", __FILE__, __LINE__);
							continue;
						}			
					elseif ($list_type == 'type')
						{
							if ((int)$value > 1) {$value = 0;}
							$request = db_query("UPDATE {$db_prefix}premiumbeat_settings SET type = {$value}
																				 WHERE myplaylist = {$check_list}", __FILE__, __LINE__);
							continue;
						}
					...
					elseif ($list_type == 'perms' && $context['lists']['perms'][0] == true)
						{
							...
							if ($context['lists']['perms'][0] == true)
								{$request = db_query("DELETE FROM `{$db_prefix}$tableName2` WHERE `{$db_prefix}$tableName2`.`permission` = '$columnName1'", __FILE__, __LINE__);}
							foreach ($context['lists']['perms'] as $value)
								{
								$value = (int)$value;									
								if ($value == 1) {$value = 999;}	
								$request = db_query("DELETE FROM `{$db_prefix}$tableName2` 
								WHERE (`{$db_prefix}$tableName2`.`ID_GROUP` LIKE '$value') AND (`{$db_prefix}$tableName2`.`permission` RLIKE '$columnName2')", __FILE__, __LINE__);																	
								if ($value != -999)
									{						
										$request = db_query("INSERT INTO `{$db_prefix}$tableName2` (`ID_GROUP`, `permission`, `addDeny`)
										VALUES ('$value' , '$columnName1', '1')", __FILE__, __LINE__);
									}		
								}	
							continue;	
						} 								
	...
	$result1 = db_query("SELECT playlist FROM {$db_prefix}premiumbeat_mp3s WHERE (tag > 0) ORDER BY tag ASC", __FILE__, __LINE__);
	while ($val = mysql_fetch_assoc($result1))
		{	
		...
		{			
			...
			if ($a == false)
				{					
					...
					$request = db_query("INSERT INTO {$db_prefix}premiumbeat_settings (`myplaylist`, `height`, `width`, `autoplay`, `type`, `skin`, `skin_type`, `autofile`, `title`, `equip`) VALUES ({$myval} , '215', '200', '1', '0', '000000', '1', false, false, '0')", __FILE__, __LINE__);
				}	
		}		
	$i = 0;	
	$result1 = db_query("SELECT myplaylist, height, width, autoplay, type, skin, skin_type, autofile, title, equip FROM {$db_prefix}premiumbeat_settings WHERE (myplaylist > 0) ORDER BY myplaylist ASC", __FILE__, __LINE__);
	while ($val = mysql_fetch_assoc($result1))
		{	
			if ((empty($val['myplaylist']))) {continue;}	 
			if ($val['myplaylist'] == 0) {continue;}					
	...
	foreach ($context['sets'] as $check)
	{
		...
		$result2 = db_query("SELECT playlist FROM {$db_prefix}premiumbeat_mp3s																				
											WHERE (playlist > 0) ORDER BY playlist ASC", __FILE__, __LINE__);
		while ($val = mysql_fetch_assoc($result2))
			{	
				if ($val['playlist'] == $checkit) {$checkit2 = false;}
			}
		mysql_free_result($result2);	
			
		if ($checkit2)
			{
				$request = db_query("DELETE FROM {$db_prefix}premiumbeat_settings WHERE myplaylist LIKE {$checkit}", __FILE__, __LINE__);
				$request = db_query("DELETE FROM {$db_prefix}premiumbeat_data WHERE my_playlist LIKE {$checkit}", __FILE__, __LINE__);
				$request = db_query("DELETE FROM `{$db_prefix}$mytableName2` WHERE `{$db_prefix}$mytableName2`.`permission` = '$perm1'", __FILE__, __LINE__);						
				$reset_page = true;
				continue;										
			}	
	}		
	
	if ($reset_page == true) {redirectexit('action=custom_mp3;sa=SettingsPremiumbeat;');}

	/* Query playlist permissions */
	$result2 = array();	
	$context['group_names'][] = array();	
	$z = 0;
	$result2 = db_query("SELECT	ID_GROUP, groupName FROM {$db_prefix}membergroups WHERE ID_GROUP > 0 ORDER BY ID_GROUP", __FILE__, __LINE__);
	while ($val = mysql_fetch_assoc($result2))
			{
				if ((int)$val['ID_GROUP'] < 1) {continue;}
				$id = $val['ID_GROUP'];
				if (empty($val['groupName'])) {$val['groupName'] = false;}				
				$context['group_names'][$id] = substr($val['groupName'], 0, 60);
				$context['groups'][$z] = substr($val['groupName'], 0, 60);
	...
	foreach ($context['sets'] as $check)
	{
		...
		foreach ($check_group as $groupid)
		{
			...
			if ((int)$groupid == -1) {$context['id_group'][$playlist] .= $txt['guest'] . ', '; continue;}			
	...
	$context['post_url'] = $scripturl . '?action=custom_mp3;sa=SettingsPremiumbeat;';
...
function PlaylistCustomMusic()
	{
		...
		$context['post_url'] = $scripturl . '?action=custom_mp3;sa=SettingsPremiumbeat;' . 'playlist=' . (int)$check_track . ';save'; 
	...
	if (empty($check_group[1])) {$check_group[1] = 0;}
		if ($check_group[1] == false) {$context['id_group'] = $txt['premiumbeat_na'];}		
		foreach ($check_group as $groupid)
		{
			if ((int)$groupid == 0 || (int)$groupid == 1) {continue;}
			if ((int)$groupid == 999) {$context['id_group'] .= $context['groups'][0] . ', '; continue;}
			if ((int)$groupid == -1) {$context['id_group'] .= $txt['guest'] . ', '; continue;}			
			$group = $context['group_names'][$groupid];	
			if ($group == false) {$context['id_group'] = $txt['premiumbeat_na'];continue;}								
		...
		$a = checkFieldPremium2('premiumbeat_settings','myplaylist');		
		if ($a == false) {fatal_lang_error('customMusic_error2', false);}	
		...
		$i = 0;
		$result1 = db_query("SELECT myplaylist, height, width, autoplay, type, skin, skin_type, autofile, title, equip FROM {$db_prefix}premiumbeat_settings WHERE (myplaylist = {$check_track}) LIMIT 1", __FILE__, __LINE__);
		while ($val = mysql_fetch_assoc($result1))
		{	
			if ((empty($val['myplaylist']))) {continue;}	 
	...
	if ($context['mysets']['type'] == 1) {$context['xtype'] = $txt['premiumbeat_asc'];}	
	if ($context['mysets']['autoplay'] == 1) {$context['xauto'] = $txt['premiumbeat_enabled'];}
	if ((!empty($context['mysets']['equip'])) && $context['mysets']['equip'] == 1) {$context['equip'] = $txt['premiumbeat_enabled'];}
	$context['toggle'] = '<script type="text/javascript">
	function changeTypeA(){
   	document.getElementById("updateDir").innerHTML = "'.$txt['premiumbeat_asc'].'";
	}
...
function UploadCustomMusic()
{
	...
	global $txt, $scripturl, $context, $db_prefix, $sourcedir, $boarddir;
	loadLanguage('Modifications');		
	$context['robot_no_index'] = true;		
	$tempDir = $boarddir . '/mp3_music/temp';
	$count = 1;	
	$musicfolder = false;	
	$fcount = 0;
	$y = 0;
	...
	$context['description'] = $txt['customMusic_tabtitle8'];
	$context['template'] = 'CustomMusicUpload';
	$context['sub_template'] = 'customMusic_upload';	
	$context['post_url'] = $scripturl . '?action=custom_mp3;sa=UploadPremiumbeat;save;';
	$context['files_list'] = !empty($context['files_list']) ? $context['files_list'] : array();
	...
	if (!$context['check'] && (int)$context['queryPlaylist'] > 0)
	{
		if (!check_user($user))
			$request = db_query("INSERT INTO {$db_prefix}premiumbeat_users (`user_id`, `last_playlist`, `pref_playlist`, `downloads`, `autoplay`) 
												VALUES ('{$user}', '{$context['queryPlaylist']}', '0', '0', '0')", __FILE__, __LINE__);	
		else
			$request = db_query("UPDATE {$db_prefix}premiumbeat_users SET last_playlist = '{$context['queryPlaylist']}' WHERE (user_id = '{$user}') LIMIT 1", __FILE__, __LINE__);
	...
	elseif (!$context['check'] && (int)$context['queryPlaylist'] < 0)
	{
		...
		{
			$request = db_query("INSERT INTO {$db_prefix}premiumbeat_users (`user_id`, `last_playlist`, `pref_playlist`, `downloads`, `autoplay`) 
												VALUES ('{$user}', '0', '0', '0', '0')", __FILE__, __LINE__);										
		}											
		else
		{
			$request = db_query("UPDATE {$db_prefix}premiumbeat_users SET last_playlist = '0' WHERE (user_id = '{$user}') LIMIT 1", __FILE__, __LINE__);			
	...
	/* Query extra data */
	$result1 = db_query("SELECT data.id, data.user_id, data.my_playlist, data.file, data.author, data.date, data.downloads, mem.ID_MEMBER,
						 mem.realName, sets.myplaylist, sets.equip, user.last_playlist, user.visibility
						 FROM {$db_prefix}premiumbeat_data AS data
						 LEFT JOIN {$db_prefix}members AS mem ON (mem.ID_MEMBER = data.user_id)
						 LEFT JOIN {$db_prefix}premiumbeat_users AS user ON (user.user_id = '{$user}')
						 LEFT JOIN {$db_prefix}premiumbeat_settings AS sets ON (sets.myplaylist = data.my_playlist)	
						 ORDER BY data.id ASC", __FILE__, __LINE__);
	while ($val = mysql_fetch_assoc($result1))	
	{	
		foreach ($columns_data as $data)	
	...
	{
		$result1 = db_query("SELECT myplaylist, autofile, title, equip, settings.filename FROM {$db_prefix}premiumbeat_settings	
											LEFT JOIN {$db_prefix}premiumbeat_forum_settings AS settings ON (settings.reference = 1)																			
											WHERE (myplaylist > 0 AND equip > 0) ORDER BY myplaylist ASC", __FILE__, __LINE__);
		while ($val = mysql_fetch_assoc($result1))		
		{	
	...
	{
		$result1 = db_query("SELECT myplaylist, title, equip, settings.filename FROM {$db_prefix}premiumbeat_settings	
											LEFT JOIN {$db_prefix}premiumbeat_forum_settings AS settings ON (settings.reference = 1)																			
											WHERE (myplaylist > 0 AND equip > 0) ORDER BY myplaylist ASC", __FILE__, __LINE__);
		while ($val = mysql_fetch_assoc($result1))	
		{		
	...
	if (!@is_dir($boarddir . '/my_music/temp'. $context['user']['id']))
		@mkdir($boarddir . '/my_music/temp'. $context['user']['id'], 0755);		
	...
	if (empty($context['sets']['eqip']))
		...
		{				
			...
			$ext = 'mp3';
			$res=$up->uploadfile('my_music/temp'. $context['user']['id'],$ext);					
		}	
		else
		{
			...
			$upfiles = ReadMusicDirectory2($boarddir . '/my_music/temp'. $context['user']['id']);		
			copyMusicDirectory($boarddir . '/my_music/temp'. $context['user']['id'], $boarddir . '/' . $context['destinationz']); 	
			deleteTempArchives($boarddir . '/my_music/temp'. $context['user']['id']);			
			...
			foreach ($upfiles as $file)
			{
				...
				{
					if (cleanPremiumbeatQuery2($checkfile) == $file)
						$check = true;
				}
				if (!$check)
				{
					...
					if ($playlist_check < 0 || $playlist_check > 99)
						$playlist_check = 0;
					$request = db_query("DELETE FROM {$db_prefix}premiumbeat_data WHERE (file LIKE '{$file}')", __FILE__, __LINE__);
					$request = db_query("INSERT INTO {$db_prefix}premiumbeat_data (`user_id`, `my_playlist`, `file`, `author`, `date`, `downloads`) 
										VALUES ('{$context['user']['id']}', '{$playlist_check}', '{$file}', '{$author}', '{$date}', '0')", __FILE__, __LINE__);															
				}			
		...
		if ((is_array($files)) && count($files) > 0)
		{
			foreach ($files as $file)
			{
				if (strtolower(substr($file, -4, 4)) == '.mp3')
				{
					...
					foreach ($checkfiles['file'] as $checkfile)
					{										
						if ($context['destinationz'] . '/' . formspecialcharsformeagain3($file, true) == formspecialcharsformeagain3($checkfile, true))
						{																			
							foreach ($columns_data as $data)
							{	
	...
	/* Adjust for compatible filenames if opted */
	if (($compat == 1 && $context['destinationz'] && allowedTo('premiumbeat_config')) && is_array($files) && is_dir($boarddir . '/' . $context['destinationz']))
	{
		...
		redirectexit($scripturl.'?action=custom_mp3;sa=UploadPremiumbeat;queryPlaylist=' . $context['playlist']); 
	}
	...
	elseif (($compat == 2 && allowedTo('premiumbeat_showdelete')) && (isset($context['delete_songs']) && is_array($context['delete_songs'])))
	{
		...
		foreach ($context['delete_songs'] as $delete_song)
		{
			...
			$request = db_query("DELETE FROM {db_prefix}premiumbeat_data WHERE (file LIKE '{$myfile}')", __FILE__, __LINE__);
		} 
		
		/* Toggle uploader visibilty for opted files */
		foreach ($context['toggle_songs'] as $toggle_song)
		{
			$toggle = $context['destinationz'] . '/' . cleanPremiumbeatQuery2($toggle_song);
			$visible = (int)$context['up_visibility'];
			$request = db_query("UPDATE {$db_prefix}premiumbeat_data SET author = MOD(author+1,2) WHERE (file = '{$toggle}' AND user_id = '{$user}') LIMIT 1", __FILE__, __LINE__);
			$result = db_query("SELECT user_id, file, author FROM {$db_prefix}premiumbeat_data WHERE (file = '{$toggle}' AND user_id = '{$user}') LIMIT 1", __FILE__, __LINE__);
			while ($val = mysql_fetch_assoc($result))	
			{		
				$visible = $val['author'];
			...
			$request = db_query("UPDATE {$db_prefix}premiumbeat_users SET visibility = '{$visible}' WHERE (user_id = '{$user}') LIMIT 1");	
		} 		
			
		/* Download opted files as a compressed zip archive */
		if(count($context['download_songs']) > 0 && allowedTo('premiumbeat_showdownload')) 
		{
			$tempdir = 'download_temp_' . $context['user']['id'] . '-' . time();
			$zipfile = 'mp3_music_' . $context['user']['id'] . '-' . time() . '.zip';
			...
			foreach ($context['download_songs'] as $download)
			{
				...
				if (@file_exists($file) && !@file_exists($boarddir . '/' . 'my_music/' . $tempdir . '/' . $download))
				{
					...
					if (!check_file($update))	
						$request = db_query("INSERT INTO {$db_prefix}premiumbeat_data (`user_id`, `my_playlist`, `file`, `author`, `date`, `downloads`) 
															VALUES ('0', '{$playlist_check}', '{$update}', 0, '{$mydate}', '1')", __FILE__, __LINE__);						
					else	
						$request = db_query("UPDATE {$db_prefix}premiumbeat_data SET downloads = downloads + 1 WHERE (file = '{$update}') LIMIT 1", __FILE__, __LINE__);
					
					if (!check_user($user))
						$request = db_query("INSERT INTO {$db_prefix}premiumbeat_users (`user_id`, `last_playlist`, `pref_playlist`, `downloads`, `autoplay`, `visibility`) 
															VALUES ('{$user}', '{$playlist_check}', 0, 1, 0, '{$context['up_visibility']}')", __FILE__, __LINE__);			
					else
						$request = db_query("UPDATE {$db_prefix}premiumbeat_users SET downloads = downloads + 1 WHERE (user_id = '{$user}') LIMIT 1", __FILE__, __LINE__);			
	...
	elseif ($compat == 3)
	{
		$request = db_query("UPDATE {$db_prefix}premiumbeat_data SET my_playlist = 0 WHERE (date = 0 AND user_id = '{$context['user']['id']}') LIMIT 1", __FILE__, __LINE__);
		redirectexit($scripturl.'?action=custom_mp3;sa=UploadPremiumbeat;queryPlaylist=-1');
	}
	...
	$context['downlimit'] = '	function downcontrol(j)
		...
		$context['musicAdmin_pages3'] = '
		<script type="text/javascript">
			<!--
			var pager = new Pager("premFiles", 15); 
			pager.init(); 
...
function createPlaylist($playlists=array())
{
	...
	if ((int)$skin_type < 1 || (int)$skin_type > 5)
		$skin_type = 1;
	$autoplay = !empty($autoplay) ? (int) $autoplay : 0;
	$type = !empty($type) ? (int) $type : 0;
	$autofile = !empty($autofile) ? $autofile : false;
	$title = !empty($title) ? $title : false;
	$equip = !empty($equip) ? (int) $equip : 0;
	/* Determine the next available playlist id else it will auto increment */	
	$result =  db_query("SELECT myplaylist FROM {$db_prefix}premiumbeat_settings WHERE (myplaylist > 0) ORDER BY myplaylist ASC", __FILE__, __LINE__);
	while ($val = mysql_fetch_assoc($result))
		{	
			if (empty($val['myplaylist'])) {continue;} 
	...
	if ((int)$mylist == 0)
		$request =  db_query("INSERT INTO {$db_prefix}premiumbeat_settings (`height`, `width`, `autoplay`, `type`, `skin`, `skin_type`, `autofile`, `title`, `equip`) VALUES ('{$height}', '{$width}', '{$autoplay}', '{$type}', '{$skin}', '{$skin_type}', '{$autofile}', '{$title}', '{$equip}')", __FILE__, __LINE__);
	else
		$request =  db_query("INSERT INTO {$db_prefix}premiumbeat_settings (`myplaylist`, `height`, `width`, `autoplay`, `type`, `skin`, `skin_type`, `autofile`, `title`, `equip`) VALUES ('{$mylist}', '{$height}', '{$width}', '{$autoplay}', '{$type}', '{$skin}', '{$skin_type}', '{$autofile}', '{$title}', '{$equip}')", __FILE__, __LINE__);													
	redirectexit('action=custom_mp3;sa=SettingsPremiumbeat;sesc='.$context['session_id']); 															
...
function check_mp3($tag)
	{
		global $db_prefix;		
		$result2 = db_query("SELECT tag FROM {$db_prefix}premiumbeat_mp3s WHERE (tag = '{$tag}') LIMIT 1", __FILE__, __LINE__);
		$result3 = mysql_num_rows($result2);	
		mysql_free_result($result2);		
		if ($result3 > 0) {return true;}
		return false;
...
function check_file($file)
	{
		global $db_prefix;			
		$result2 = db_query("SELECT file FROM {$db_prefix}premiumbeat_data WHERE (file = '{$file}') LIMIT 1", __FILE__, __LINE__);
		$result3 = mysql_num_rows($result2);	
		mysql_free_result($result2);		
		if ($result3 > 0) {return true;}
		return false;
...
function check_user($user)
	{
		global $db_prefix;		
		$result2 = db_query("SELECT user_id FROM {$db_prefix}premiumbeat_users WHERE (user_id = '{$user}') LIMIT 1", __FILE__, __LINE__);
		$result3 = mysql_num_rows($result2);	
		mysql_free_result($result2);		
		if ($result3 > 0) {return true;}
		return false;
...
function createpremiumval_settings($tableName, $columnName, $value, $playlist) 
	{
		...
		$request = db_query("UPDATE {$db_prefix}$tableName SET $columnName = '{$value2}' WHERE `{$db_prefix}$tableName`.`myplaylist` = {$playlist} LIMIT 1", __FILE__, __LINE__);
...
function createpremiumval_mp3($tableName, $columnName, $tag, $value) 
	{
		...
		if (empty($value)) {$value = false;}
		$value2 = cleanPremiumbeatQuery2($value);
		$request = db_query("UPDATE {$db_prefix}$tableName SET $columnName = '{$value2}' WHERE `{$db_prefix}$tableName`.`tag` = {$tag} LIMIT 1", __FILE__, __LINE__);
